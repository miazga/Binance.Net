...
{
    private readonly Func<string, T> _clientFunc;
        ...
        Dictionary<string, List<string>> ignoreProperties = null)
    {
        ...
        List<string> skippedMethods = new();
        ...
        foreach (MethodInfo method in callResultMethods)
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                try
                {
                    file = File.OpenRead(Path.Combine(path, "JsonResponses", folderPrefix,
                        $"{method.Name}{(i == 0 ? "" : i.ToString())}.txt"));
                }
                ...
                // act
                CallResult result =
                    (CallResult)await TestHelpers.InvokeAsync(method, getSubject(client), input.ToArray());

                // asset
                Assert.Null(result.Error, method.Name);

                object resultData = result.GetType().GetProperty("Data", BindingFlags.Public | BindingFlags.Instance)
                    .GetValue(result);
                ProcessData(method.Name + (i == 0 ? "" : i.ToString()), resultData, json, parametersToSetNull,
                    useNestedJsonPropertyForCompare, ignoreProperties);
            }
        }
    ...
    internal static void ProcessData(string method, object resultData, string json, string[] parametersToSetNull = null,
        ...
        Dictionary<string, List<string>> ignoreProperties = null)
    {
        IEnumerable<(PropertyInfo p, JsonPropertyAttribute)> resultProperties = resultData.GetType().GetProperties()
            .Select(p => (p,
                (JsonPropertyAttribute)p.GetCustomAttributes(typeof(JsonPropertyAttribute), true).SingleOrDefault()));
        ...
        if (resultData.GetType().GetInterfaces().Contains(typeof(IDictionary)))
        {
            ...
            foreach (JProperty dictProp in properties)
            {
                ...
                {
                    throw new Exception(
                        $"{method}: Dictionary has no value for {dictProp.Name} while input json `{dictProp.Name}` has value {dictProp.Value}");
                }
                ...
                else
                {
                    if (dict[dictProp.Name] == default && dictProp.Value.Type != JTokenType.Null)
                    {
                        ...
                        throw new Exception(
                            $"{method}: Dictionary entry `{dictProp.Name}` has no value while input json has value {dictProp.Value}");
                    }
                }
            }
        }
        else if (jsonObject.Type == JTokenType.Array)
        {
            ...
            foreach (JToken jObj in jObjs)
            {
                ...
                if (jObj.Type == JTokenType.Object)
                {
                    ...
                    {
                        if (ignoreProperties?.ContainsKey(method) == true &&
                            ignoreProperties[method].Contains(subProp.Name))
                        {
                            continue;
                        }
                ...
                else if (jObj.Type == JTokenType.Array)
                {
                    ...
                    IEnumerable<(PropertyInfo p, ArrayPropertyAttribute?)> resultProps = resultObj.GetType()
                        .GetProperties().Select(p => (p,
                            p.GetCustomAttributes(typeof(ArrayPropertyAttribute), true).Cast<ArrayPropertyAttribute>()
                                .SingleOrDefault()));
                    object arrayConverterProperty = resultObj.GetType()
                        .GetCustomAttributes(typeof(JsonConverterAttribute), true).FirstOrDefault();
                    ...
                    foreach (JToken item in jObj.Values())
                    {
                        ...
                        if (arrayProp != null)
                        {
                            CheckPropertyValue(method, item, arrayProp.GetValue(resultObj), arrayProp.Name,
                                "Array index " + i, arrayProp, ignoreProperties);
                        }
    ...
    private static void CheckObject(string method, JProperty prop, object obj,
        Dictionary<string, List<string>> ignoreProperties)
    {
        IEnumerable<(PropertyInfo p, JsonPropertyAttribute)> resultProperties = obj.GetType().GetProperties()
            .Select(p => (p,
                (JsonPropertyAttribute)p.GetCustomAttributes(typeof(JsonPropertyAttribute), true).SingleOrDefault()));
        ...
        if (property is null)
        {
            property = resultProperties
                .SingleOrDefault(p => p.p.Name.ToUpperInvariant() == prop.Name.ToUpperInvariant()).p;
        }
    ...
    private static void CheckPropertyValue(string method, JToken propValue, object propertyValue,
        string propertyName = null, string propName = null, PropertyInfo info = null,
        Dictionary<string, List<string>> ignoreProperties = null)
    {
        if (propertyValue == default && propValue.Type != JTokenType.Null &&
            !string.IsNullOrEmpty(propValue.ToString()))
        {
            ...
            {
                throw new Exception(
                    $"{method}: Property `{propertyName}` has no value while input json `{propName}` has value {propValue}");
            }
        }

        if ((propertyValue == default &&
             (propValue.Type == JTokenType.Null || string.IsNullOrEmpty(propValue.ToString()))) ||
            propValue.ToString() == "0")
        {
            return;
        }
            ...
            foreach (JProperty dictProp in properties)
            {
                ...
                else
                {
                    if (dict[dictProp.Name] == default && dictProp.Value.Type != JTokenType.Null)
                    {
                        // Property value not correct
                        throw new Exception(
                            $"{method}: Dictionary entry `{dictProp.Name}` has no value while input json has value {propValue} for");
                    }
                }
            }
        }
                 ...
                 && propertyValue.GetType() != typeof(string))
        {
            ...
            {
                enumerator.MoveNext();
                object[] typeConverter = enumerator.Current.GetType()
                    .GetCustomAttributes(typeof(JsonConverterAttribute), true);
                if (typeConverter.Any() && ((JsonConverterAttribute)typeConverter.First()).ConverterType !=
                ...
                else
                {
                    ...
                    {
                        throw new Exception(
                            $"{method}: Property `{propertyName}` has no value while input json `{propName}` has value {jtoken}");
                    }
        ...
        else
        {
            if (propValue.Type == JTokenType.Object)
            {
                foreach (JToken item in propValue)
                {
                    ...
                    {
                        if (ignoreProperties?.ContainsKey(method) == true &&
                            ignoreProperties[method].Contains(prop.Name))
                        {
                            continue;
                        }
    ...
    private static void CheckValues(string method, string property, JValue jsonValue, object objectValue)
    {
        if (jsonValue.Type == JTokenType.String)
        {
            ...
            {
                throw new Exception(
                    $"{method}: {property} not equal: {jsonValue.Value<string>()} vs {objectValue.ToString()}");
            }
        }
        else if (jsonValue.Type == JTokenType.Integer)
        {
            ...
            {
                throw new Exception(
                    $"{method}: {property} not equal: {jsonValue.Value<long>()} vs {Convert.ToInt64(objectValue)}");
            }
        }
        else if (jsonValue.Type == JTokenType.Boolean)
        {
            ...
            {
                throw new Exception(
                    $"{method}: {property} not equal: {jsonValue.Value<bool>()} vs {(bool)objectValue}");
            }
        }
    }
}