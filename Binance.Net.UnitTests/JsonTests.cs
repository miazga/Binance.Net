...
{
    private readonly JsonToObjectComparer<IBinanceClient> _comparer = new(json => TestHelpers.CreateResponseClient(json,
        new BinanceClientOptions
        {
            ApiCredentials = new ApiCredentials("123", "123"),
            SpotApiOptions =
                new BinanceApiClientOptions { RateLimiters = new List<IRateLimiter>(), AutoTimestamp = false },
            UsdFuturesApiOptions =
                new BinanceApiClientOptions { RateLimiters = new List<IRateLimiter>(), AutoTimestamp = false },
            CoinFuturesApiOptions = new BinanceApiClientOptions
            {
                RateLimiters = new List<IRateLimiter>(), AutoTimestamp = false
            }
        }));
    ...
    public async Task ValidateSpotExchangeDataCalls()
    {
        await _comparer.ProcessSubject(
            ...
            useNestedJsonPropertyForCompare: new Dictionary<string, string> { { "GetTradingStatusAsync", "data" } },
            parametersToSetNull: new[] { "limit" });
    }
    ...
    public async Task ValidateSpotTradingCalls()
    {
        await _comparer.ProcessSubject(
            ...
            useNestedJsonPropertyForCompare: new Dictionary<string, string>
            {
                { "GetC2CTradeHistoryAsync", "data" }, { "GetPayTradeHistoryAsync", "data" }
            },
    ...
    public async Task ValidateSpotSubAccountCalls()
    {
        await _comparer.ProcessSubject(
            ...
            useNestedJsonPropertyForCompare: new Dictionary<string, string>
            {
                { "GetSubAccountsAsync", "subAccounts" }, { "GetSubAccountAssetsAsync", "balances" }
            });
    }
